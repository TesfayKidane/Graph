...
/**
 * Graph class implements the simple undirected graph data type. Vertices and
 * edges are each stored in a linked list, and adjacency is managed with an
 * adjacency list, represented as a HashMap.
 *
 * The Graph class provides instance variables for a spanning tree and a list of
 * connected components (where each component is itself an instance of the Graph
 * class). The Graph class also has a flag to indicate whether it is bipartite.
 *
 * A Graph instance can be constructed by passing in one of two representations
 * of graph data: as an array of Edges (used internally when getting a spanning
 * tree or connected components) or as a List of Pairs, where a Pair A,B
 * signifies vertices A and B as endpoints of an edge.
 *
 * The public method loadMajorStructures is a top-level method for creating all
 * the auxiliary structures supported by the Graph, including spanning tree,
 * connected components, bipartite flag. (Initially, only finding spanning tree
 * is supported.) If a user attempts to access one of the cached structures for
 * a Graph object (such as a spanning tree) before loadMajorStructures has been
 * run, loadMajorStructures will be run first so that the required data will be
 * available.
 */
public class Graph {
    ...
    /**
     * Constructs a Graph object from an array of Edges. Used internally to
     * create a spanning tree or a set of connected components.
     */
    ...
    /**
     * Constructs a graph from a list of pairs. A pair (A,B) is transformed into
     * vertices A and B together with an edge A-B.
     */
    ...
    private void loadMajorStructures() {
        ...
        connectedComponents = conComp.computeConnnectedComponents();
    ...
    */
     ...
     * map) is deg v. Also, each vertex is processed (cloned and the clone is
     * added to its list); processing is O(1) Therefore, it is the sum over v in
     * V of 1 + deg(v), which is O(n+m).
     */
